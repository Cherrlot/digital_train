...
class _CarouselState extends State<VipBanner> with WidgetsBindingObserver {
  ...
  * */
  void _onPageChanged(int index) {
    ...
    var position = 0;
    if(length > 0) {
      position = index % widget.imageList!.length;
    }
    setState(() {
  ...
  * */
  Widget getBannerImage(String imageUrl) {
    //不需要占位图
    if (widget.placeholderImage == null && imageUrl.isNotEmpty) {
  ...
  * */
  ImageProvider getPlaceholder() {
    return const AssetImage(ImageConstant.imageBanner);
  }
  ...
  * */
  Widget bannerTitle() {
    var text = "";
    if(widget.titleList != null && widget.titleList!.isNotEmpty) {
      text = widget.titleList![_currentPage];
    }
    return Positioned(
